...
## 任务清单
- [x] `frontend/Modules/login/events.js`：调用 `/api/auth/login`，保存 JWT。
- [x] `frontend/Modules/signup/events.js`：调用 `/api/auth/signup`，保存 JWT。
- [x] `frontend/Modules/accountSetting/events.js`：调用 `/api/auth/change_password` 与 `/api/auth/change_username`。
