...
const DEFAULT_DEVICE_ID = 'web-phase3-device';
...
function mapApiError(error: unknown): { code: string | null; message: string } {
  if (error instanceof ApiRequestError) {
    ...
    if (code === 'suggestions_not_processed') {
      return {
        code,
        message: error.message || 'Process all sentence comments before submitting.'
      };
    }
...
export function useWorkDetail(workId: string | undefined) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [work, versionList] = await Promise.all([
        getWork(workId),
  ...
  const openVersion = useCallback(async (versionNumber: number) => {
    if (!workId) {
      return;
    }
  ...
  const revert = useCallback(async (targetVersion: number) => {
    if (!workId) {
      return;
    }
